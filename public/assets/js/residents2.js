document.addEventListener('DOMContentLoaded',()=>{
    ...
    const applyFilters = ()=>{
        ...
        filteredResults.style.display = (filteredRows.length !== rows.length || searchInput.value || statusFilter.value || genderFilter.value || ageMin.value || ageMax.value || purokFilter.value || barangayFilter.value) ? 'block':'none';
        currentPage = 1;
        renderTable();
    };
    ...
    nextPageBtn.addEventListener('click', ()=>{const totalPages=Math.ceil(filteredRows.length/pageSize); if(currentPage<totalPages) currentPage++; renderTable();});
